...
|         Tim Thorpe <blushift@netins.net>                             |
+----------------------------------------------------------------------+
*/


/**
 * This Class implements the 56-bit DES encryption algorithm,
 * created from scratch based on interpretation of the original
 * specification document.
 *
 * DES::encryptECB( "my key", "my text" );
 * and
...
class DES extends PEAR
{
	 ...
	 * @static
	 */
	function encryptECB( $key, $clearText ) 
	{
	    if ( strlen( $key ) < 7 )
	        $key = str_pad( substr( $key, 0, 7 ), 7, chr( 0x00 ) );  //We need 7 bytes for a key; no more, no less
	        
	    if ( strlen( $key ) == 7 ) 
			$key = DES::_addParity( $key );
	
	    $keys = DES::makeSubKeys( DES::bitsToBytes( $key ) ); // Chew the key into the subkeys needed for DES
	    ...
	    for ( $i = 0; $i < $blockCount; $i++ ) 
		{
	        ...
	        $cypherBlock  = DES::blockEncode( DES::bitsToBytes( $clearBlock ), $keys ); // Encrypt it
	        $cypherText  .= DES::bytesToBits( $cypherBlock ); // Convert the result back to a useful form
	    }
	 ...
	 * @static
	 */
	function blockEncode( $clearText, $subKeys ) 
	{
	    $ip_table = array(
		...
		{
	        $l[$index] = $r[$index - 1];
	        $r[$index] = DES::_xor( $l[$index - 1], DES::transform( $r[$index - 1], $subKeys[$index] ) );
	    }
	...
	function makeSubKeys( $key ) 
	{
		...
		{
	        $index++;
	        $c[$index] = DES::_rotateLeft( $c[$index - 1], $positions );
	        $d[$index] = DES::_rotateLeft( $d[$index - 1], $positions );
	    }
	...
	function transform( $data, $key ) 
	{
	    ...
	    $nybbles = array(
			chr( 0x00 ) . chr( 0x00 ) . chr( 0x00 ) . chr( 0x00 ), chr( 0x00 ) . chr( 0x00 ) . chr( 0x00 ) . chr( 0x01 ),
	 ...
	 * @static
	 */
	function bytesToBits( $byteStream ) 
	{
	 ...
	 * @static
	 */	
	function _rotateLeft( $input, $positions ) 
	{
