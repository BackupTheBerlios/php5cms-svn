...
<title>RDF API for PHP online demo</title>
...
if ( !isset( $_POST['submit'] ) || ( strlen( $_POST['RDF'] ) > 10000 ) ) 
{

?>
<form method="post" action="<?php echo $HTTP_SERVER_VARS['PHP_SELF']; ?>"> 
<p>You can paste RDF code into the text field below and choose how the data should be processed. It's possible to parse, serialize, reify and query the data.</p>
...
if ( $_POST['query_subject'] != '' || $_POST['query_predicate'] != '' || $_POST['query_object'] != '' )
{
	...
	if ( $_POST['query_object'] != '' )
	{
		...
		else if ( $_POST['object_kind'] == 'literal' ) 
		{
	...
	if ( isset( $_POST['reify'] ) && $_POST['reify'] == "1" ) 
	{
		$model =& $model->reify();
	...
	if ( $_POST['serialize'] == '1' )
	{
		...
		if ( isset( $_POST['serial_attributes'] ) && $_POST['serial_attributes'] == '1' ) 
		{
			$ser->configUseAttributes( true );
			$msg_string .= 'Use Attributes ';
		}
		
		if ( isset( $_POST['serial_entities'] ) && $_POST['serial_entities'] == '1' ) 
		{
			$ser->configUseEntities( true );
			$msg_string .= 'Use XML Entities ';
		}
		
		if ( isset( $_POST['serial_wo_qnames'] ) && $_POST['serial_wo_qnames'] == '1' ) 
		{
	...
	if ( isset( $_POST['view_dump'] ) && $_POST['view_dump'] == '1' ) 
	{
...
<a href="<? echo $HTTP_SERVER_VARS['PHP_SELF'] ?>"><h2>Go back to input form.</h2></a>
