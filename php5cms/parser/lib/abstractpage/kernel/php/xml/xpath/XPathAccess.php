...
class XPathAccess extends PEAR
{
    /**
    ...
    function load_file( $file )
    {
        ...
        if ( file_exists( $file ) && is_readable( $file ) )
        {
            ...
            if ( !empty( $content ) )
            {
                ...
                if ( !xml_parse( $parser, $content, true ) )
                {
                    ...
                    return $this->error( "XML error - file %s, line %d: %s", $file, xml_get_current_line_number( $parser ), xml_error_string( xml_get_error_code( $parser ) ) );
                }
        ...
        else
        {
            ...
            return $this->error( "File could not be found or read." );
        }
    }
    ...
    function get_axis( $step, $node )
    {
        ...
        {
            // Display an error message.
			return $this->error( "While parsing an XPath expression, an invalid axis was found: " . $axis["axis"] . ", step " . str_replace( $step, "<b>" . $step . "</b>", $this->xpath ) );
        }
    ...
    function search_string( $term, $expression )
    {
        ...
        {
            // Display an error message.
			return $this->error( "While parsing an XPath expression, there was an invalid number of brackets - predicate " . str_replace( $term, "<b>" . $term . "</b>", $this->xpath ) );
        }
    ...
    function evaluate_step( $context, $steps )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // Display an error message.
					return $this->error( "While parsing an XPath expression, the following axis could not be handled, because this version does not support this axis: " . $axis["axis"] );
                }
    ...
    function evaluate_function( $function, $arguments, $node )
    {
        ...
        {
            // Display an error message.
			return $this->error( "While parsing an XPath expression, the following function could not be handled, because this version does not support this function: " . $function );
        }
    ...
    function evaluate_predicate( $node, $predicate )
    {
        ...
        if ( $position > 0 )
        {
            ...
            switch ( $operator )
            {
                ...
                case " div ":
                    ...
                    {
                        // Display an error message.
                        return $this->error( "While parsing an XPath predicate, a error due a division by zero occured." );
                    }
                    else
                    {
        ...
        if ( ereg( "\(", $predicate ) )
        {
            ...
            {
                // Display an error message.
				return $this->error( "While parsing an XPath expression there was found an error in the following predicate, because after a closing bracket there was found something unknown: " . $predicate );
            }
            ...
            else if ( $this->is_function( $before ) )
            {
            ...
            {
                // Display an error message.
				return $this->error( "While parsing a predicate in an XPath expression, a function was found, which is not yet supported by the parser: " . str_replace( $before, "<b>" . $before . "</b>", $this->xpath ) );
            }
        }
    ...
    function check_node_test( $context, $node_test )
    {
        ...
        if ( ereg( "\(", $node_test ) )
        {
            ...
            switch ( $function )
            {
                ...
                default:
                    // Display an error message.
                    return $this->error( "While parsing an XPath expression there was found an undefined function called " . str_replace( $function, "<b>" . $function . "</b>", $this->xpath ) );
            }
        }
        else if ( $node_test == "*" )
        {
        ...
        else if ( ereg( "^[a-zA-Z0-9\-_]+", $node_test ) )
        {
        ...
        {
            // Display an error message.
            return $this->error( "While parsing the following XPath expression, an empty and therefore invalid node-test has been found: " . $this->xpath );
        }
    ...
    function handle_axis_attribute( $axis, $context )
    {
        ...
        else if ( !empty( $this->nodes[$context]["attributes"][$axis["node-test"]] ) )
        {
            // Add this node to the node-set.
    ...
    function error( $message )
    {
		...
		return PEAR::raiseError( $out, null, PEAR_ERROR_TRIGGER );
    }
} // END OF XPathAccess
