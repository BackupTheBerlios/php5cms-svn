...
class ATL_Attribute_I18N_Attributes extends ATL_Attribute
{
    ...
    function activate(&$g, &$tag)
    {
        ...
        foreach ($attributes as $key=>$value) {
            ...
            if (!in_array($test_key, $this->_overwritten)) {
                ...
                if ($tag->_parser->_outputMode() == ATL_TEMPLATE_XHTML
                    && in_array($test_key, $_atl_xhtml_boolean_attributes)) {
                    $g->doPrintString(' '.$key);
                } else {
                    $g->doPrintString(' '.$key.'="');
    ...
    function _printAttribute(&$g, &$tag, $realName, $varName, $default=false)
    {
        ...
        if ($tag->_parser->_outputMode() == ATL_TEMPLATE_XHTML
            && in_array($realName, $_atl_xhtml_boolean_attributes)) {
