 ...
 * Usage:
 *
 * $address_string = 'My Group: "Markus Nix" <mnix@docuverse> (A comment), ted@test.com (Ted Bloggs), Barney;';
 * $structure = MailRFC822::parseAddressList( $address_string, 'example.com', true )
 * print_r( $structure );
 *
 * @package peer_mail
 */

class MailRFC822 extends PEAR
{
    /**
    ...
    */
     ...
     * 
     * @return object MailRFC822 A new MailRFC822 object.
     */
    function MailRFC822( $address = null, $default_domain = null, $nest_groups = null, $validate = null, $limit = null )
    {
		$this->default_domain = 'localhost';
    ...
    function parseAddressList( $address = null, $default_domain = null, $nest_groups = null, $validate = null, $limit = null )
    {
        ...
        if ( $this->address === false || isset( $this->error_message ) )
			return PEAR::raiseError( $this->error_message );

        // Reset timer since large amounts of addresses can take a long time to get here.
        set_time_limit( 30 );

        // Loop through all the addresses.
        for ( $i = 0; $i < count( $this->addresses ); $i++ )
		{
            if ( ( $return = $this->_validateAddress( $this->addresses[$i])) === false || isset( $this->error_message ) )
				return PEAR::raiseError( $this->error_message );
    ...
    function _splitAddresses( $address )
    {
        ...
        if ( $this->_isGroup( $address ) && !isset( $this->error_message ) )
		{
            $split_char = ';';
            $is_group   = true;
        }
		else if ( !isset( $this->error_message ) )
		{
            $split_char = ',';
            $is_group   = false;
        }
        ...
        if ( $is_group )
		{
			...
			{
                $this->error_message = 'Invalid address: ' . $string;
                return false;
            }
    ...
    function _splitCheck( $parts, $char )
    {
        ...
        for ( $i = 0; $i < count( $parts ); $i++ )
		{
				 ...
				 substr( $string, -1 ) == '\\' )
			{
				...
				{
                    $this->error_message = 'Invalid address spec. Unclosed bracket or quotes';
                    return false;
                }
            }
			else
    ...
    function _hasUnclosedBrackets( $string, $chars )
    {
		...
		{
            $this->error_message = 'Invalid address spec. Unmatched quote or bracket (' . $chars . ')';
            return false;
        }
		else
		{
    ...
    function _validateAddress( $address )
    {
        ...
        if ( $address['group'] )
		{
			...
			{
                $this->error_message = 'Group name did not validate.';
                return false;
            }
			else
			{
        ...
        if ( !isset( $addresses ) )
		{
            $this->error_message = 'Empty group.';
            return false;
        }
