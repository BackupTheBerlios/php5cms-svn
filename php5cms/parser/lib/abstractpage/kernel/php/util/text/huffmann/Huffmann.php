...
class Huffmann extends PEAR
{
	...
	/**
	 * Lenght of the input carrier at any given time 
	 *
	 * @access private
	 */
	var $_icarlen;
	...
	/**
	 * Lenght of the output carrier at any given time 
	 *
	 * @access private
	 */
	var $_ocarlen;
	...
	{
		if ( trim( $ifile ) == "" )
			return PEAR::raiseError( "No input file provided." );
		else
			$this->_ifile = $ifile;
	
		if ( trim( $ofile ) == "" )
			return PEAR::raiseError( "No output file provided." );
		else
			$this->_ofile = $ofile;
		...
		if ( !( $this->_ifhand = @fopen( $this->_ifile, "rb" ) ) )
			return PEAR::raiseError( "Unable to open input file." );
	
		if ( !( $this->_ofhand = @fopen( $this->_ofile, "wb" ) ) )
			return PEAR::raiseError( "Unable to open output file." );
