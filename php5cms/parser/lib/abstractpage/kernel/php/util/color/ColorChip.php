...
/**
 * A constant used to indicate that RGB color information 
 * is to be used by certain methods.
 */
define( 'COLORCHIP_RGB', 'RGB' );

/**
 * A constant used to indicate that HSV color information 
 * is to be used by certain methods.
 */
define( 'COLORCHIP_HSV', 'HSV' );

/**
 * A constant used to indicate that Hexadecimal color information 
 * is to be used by certain methods.
 */
define( 'COLORCHIP_HEX', 'HEX' );

/**
 * The COLORCHIP_WEBSAFE_ALT constant is used by the getWebSafeDither 
 * and getNearestWebSafeComponent method.
 */
define( 'COLORCHIP_WEBSAFE_ALT', 'WEBSAFE_ALT' );


/**
 * ColorChip is a PHP class for manipulating colors using 
 * RGB, HSV, and Hexadecimal color models.
 *
...
class ColorChip extends PEAR
{
	/** 
	 * The object's Hue property.  Value can be 0-360, or boolean FALSE for no hue (Grayscale) 
	 * @var int
	 */
	var $h; 
	 ...
	 * This should be one of the constants COLORCHIP_RGB, COLORCHIP_HSV, or COLORCHIP_HEX.  
	 * Default is COLORCHIP_RGB (Where $arg1 is R, $arg2 is G, and $arg3 is B.) 
	 * If $type is set to COLORCHIP_HEX, then $arg1 should contain the full hex triplet 
	 * (EG 'FFFFFF'), while $arg2 and $arg3 can be null.
	 * @access public
	 */
	function ColorChip( $arg1, $arg2, $arg3, $type = COLORCHIP_RGB )
	{
		switch ( $type )
		{
			case COLORCHIP_HSV:
				...
				$this->s = $arg2;
				$this->v = $arg3;
				$this->_updateAllColorInfo( COLORCHIP_HSV );
			...
			case COLORCHIP_RGB:
				...
				$this->g = $arg2;
				$this->b = $arg3;
				$this->_updateAllColorInfo( COLORCHIP_RGB );
				
				break;

			case COLORCHIP_HEX:
				$this->hex = $arg1;
				$this->_updateAllColorInfo( COLORCHIP_HEX );
	 ...
	 * @access public
  	...
  	function clone()
	{
    	$newColor = new ColorChip( $this->r, $this->g, $this->b, COLORCHIP_RGB );
    	return $newColor;
  	}
	 ...
	 * If $which is set to COLORCHIP_WEBSAFE_ALT, the second-closest web safe value will
	 ...
	 * @param string $which Which websafe match to return; by default, the method returns the closest websafe match, but if $which is set to COLORCHIP_WEBSAFE_ALT, the method will return the second closest match; this is useful when attempting to approximate a color by dithering two web-safe colors.
  	...
  	function getNearestWebSafeComponent( $rgbComponent, $which = '' )
	{
		...
		if ( !in_array( $comp, $safe ) )
		{
			...
			{
				// If the component is between the current pair of websafe percentages
				if ( $pair[0] < $comp && $comp < $pair[1] )
				{ 
	  				if ( $comp - $pair[0] > $pair[1] - $comp )
					{
						// If the alternative websafe color is desired
	    				if ( $which == COLORCHIP_WEBSAFE_ALT )
						{
							$comp = $pair[0];
						}
						else
  	...
  	function getWebSafeDither()
	{
		...
		$color2->r = ColorChip::getNearestWebSafeComponent( $color2->r, COLORCHIP_WEBSAFE_ALT );
    	$color2->g = ColorChip::getNearestWebSafeComponent( $color2->g, COLORCHIP_WEBSAFE_ALT );
    	$color2->b = ColorChip::getNearestWebSafeComponent( $color2->b, COLORCHIP_WEBSAFE_ALT );
  	...
  	function _updateAllColorInfo( $useInfo )
	{
    	switch ( $useInfo )
		{
   	 		case COLORCHIP_HSV: // Update RGB and Hex values using the contents of h,s, and v.
      			$this->_updateRgb();
      			$this->_updateHex();
      
	  			break;
      
    		case COLORCHIP_RGB: // Update HSV and Hex values using the contents of R,G, and B.
      			$this->_updateHex();
      			$this->_updateHsv();
      
	  			break;
      
    		case COLORCHIP_HEX: // Update RGB and HSV values using the contents of Hex.
      			$this->_updateRgb( COLORCHIP_HEX );
      			$this->_updateHsv();
  	...
  	/** 
	 * Updates the object's r, g, and b properties. if $useInfo is COLORCHIP_HSV, uses 
	 * the object's HSV values, if $useInfo is COLORCHIP_HEX, uses the object's hex property.
	 * 
	 * @return void
	 * @access private
	 */
  	function _updateRgb( $useInfo = COLORCHIP_HSV )
	{
    	switch ( $useInfo )
		{
    		case COLORCHIP_HSV:
