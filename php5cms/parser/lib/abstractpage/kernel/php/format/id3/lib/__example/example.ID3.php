...
echo '<HTML><HEAD><TITLE>ID3Lib Example</TITLE></HEAD><BODY>';
...
if ( isset( $_REQUEST['filename'] ) ) 
{
	...
	{
		$MP3fileInfo = ID3::getAllMP3info( $_REQUEST['filename'], $_REQUEST['assumeFormat'] );
	} 
	else 
	{
		$MP3fileInfo = ID3::getAllMP3info( $_REQUEST['filename'], '' );
		
		if ( !isset( $MP3fileInfo['fileformat'] ) || ( $MP3fileInfo['fileformat'] == '' ) ) 
		{
			$formatExtensions = array(
				'mp3' => 'mp3', 
				'ogg' => 'ogg', 
				'zip' => 'zip', 
				'wav' => 'riff', 
			...
			if ( isset( $formatExtensions[ID3::fileextension( $_REQUEST['filename'] )] ) )
				$MP3fileInfo = ID3::getAllMP3info( $_REQUEST['filename'], $formatExtensions[ID3::fileextension( $_REQUEST['filename'] )] );
		}
	}
	...
	echo ID3::tableVarDump( $MP3fileInfo );
...
else 
{
	...
	if ( $handle = @opendir( $listdirectory ) ) 
	{
		...
		while ( $file = readdir( $handle ) ) 
		{
			...
			{
				$fileinformation = ID3::getAllMP3info( $currentfilename, false );
				// $fileinformation = ID3::getBasicMP3info( $currentfilename, false, true, false );
				...
				if ( !isset( $fileinformation['fileformat'] ) || ( $fileinformation['fileformat'] == '' ) ) 
				{
					...
					if ( isset( $formatExtensions[ID3::fileextension( $currentfilename )] ) ) 
					{
						$fileinformation = ID3::getAllMP3info( $currentfilename, $formatExtensions[ID3::fileextension( $currentfilename )]  );
						// $fileinformation = ID3::getBasicMP3info( $currentfilename, false, true, false );
					}
				}
		...
		foreach ( $DirectoryContents as $dirname => $val ) 
		{
			...
			if ( isset( $DirectoryContents["$dirname"]['known'] ) && is_array( $DirectoryContents["$dirname"]['known'] ) ) 
			{
				...
				foreach ( $DirectoryContents["$dirname"]['known'] as $filename => $fileinfo ) 
				{
					...
					echo '<TD ALIGN="RIGHT">&nbsp;' . ( isset( $fileinfo['bitrate'] )? ID3::bitrateText( $fileinfo['bitrate'] / 1000 ) : '-' ) . '</TD>';
					...
					if ( isset( $fileinfo['id3']['id3v1'] ) ) 
					{
						...
						{
							echo '<TD ALIGN="LEFT">&nbsp;<SPAN STYLE="color: ' . ( ID3::ID3v1matchesID3v2( $fileinfo['id3']['id3v1'], $fileinfo['id3']['id3v2'] )? '#009900;">matches' : '#CC0000;">differs from' ) . ' ID3v2</SPAN></TD>';
							echo '<TD ALIGN="LEFT">&nbsp;<SPAN STYLE="color: ' . ( ID3::ID3v1matchesID3v2( $fileinfo['id3']['id3v1'], $fileinfo['id3']['id3v2'] )? '#009900;">matches' : '#CC0000;">differs from' ) . ' ID3v1</SPAN></TD>';
						} 
						else 
						{
