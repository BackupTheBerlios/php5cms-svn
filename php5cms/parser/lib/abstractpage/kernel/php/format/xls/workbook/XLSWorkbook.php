...
using( 'format.xls.workbook.XLSFormat' );
using( 'format.xls.workbook.XLSWorksheet' );
using( 'format.xls.workbook.XLSOLEwriter' );
...
class XLSWorkbook extends XLSBiffWriter
{
	/**
	 * Constructor
	 */
  	function XLSWorkbook( $filename )
    {
    	$this->XLSBiffWriter();
	...
	/**
	 * Calls finalization methods and explicitly close the XLSOLEwriter file
	 * handle.
	 */
  	function close()
    {
    ...
    {
    	$format = new XLSFormat( $this->xf_index, $properties );
    	$this->xf_index += 1;
    	$this->formats[] = &$format;
  	...
  	function set_custom_color( $index, $red, $green, $blue )
    {
    	...
    	if ( $index < 8 || $index > 64 )
			return PEAR::raiseError( "Color index outside range: 8 <= index <= 64." );
    	...
    	if ( ( $red   < 0 || $red   > 255 ) ||
         	 ...
         	 ( $blue  < 0 || $blue  > 255 ) )  
		{
        	return PEAR::raiseError( "Color component outside range: 0 <= color <= 255." );
        }
    ...
    {
    	$OLE  = new XLSOLEwriter( $this->filename );
