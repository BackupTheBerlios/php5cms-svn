...
class SysInfo_Linux extends SysInfo
{
	/**
	 * Constructor
	 *
    ...
    function users()
    {
        $who = split( '=', Util::executeProgram( 'who', '-q' ) );
        $result = $who[1];
    ...
    function cpu_info()
    {
		...
		{
            if ( !ArrayUtil::inArray( $ar_buf[1], $keys ) )
        ...
        while ( $file = readdir( $handle ) ) 
		{
            if ( preg_match( '/^hd/', $file ) ) 
			{
                ...
                if ( $fd = fopen("/proc/ide/$file/model", 'r' ) ) 
				{
                    ...
                    else if ( preg_match( '/IBM/', $results[$file]['model'] ) ) 
                        $results[$file]['manufacture'] = 'IBM';
                    else if ( preg_match( '/FUJITSU/', $results[$file]['model'] ) ) 
	...
	/**
	 * Return an array of informtaion about network connections.
	 *
	 * @access public
	 */
    function sys_connections()
    {
        $netstat     = Util::executeProgram( 'netstat', '-n' );
        $connections = explode( "\n", $netstat );
        $return      = array();
