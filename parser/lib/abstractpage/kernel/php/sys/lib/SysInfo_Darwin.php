...
class SysInfo_Darwin extends SysInfo
{
	/**
	 * @access public
	 */
    ...
    {
        $s = Util::executeProgram( 'sysctl', $key );
    ...
    function memory()
    {
        ...
        $results['ram'] = array();
        $pstat = Util::executeProgram( 'vm_stat' ); // use darwin's vm_stat
        $lines = split( "\n", $pstat );
    ...
    function network()
    {
        $netstat = Util::executeProgram( 'netstat', '-nbdi | cut -c1-24,42- | grep Link' );
        $lines   = split( "\n", $netstat );
        $results = array();
