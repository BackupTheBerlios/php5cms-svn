...
+----------------------------------------------------------------------+
|Authors: Enrique Garcia M. <egarcia@egm.as>                           |
...
class ExcelFinance extends PEAR
{
	/**
	...
	function DATEADD( $datepart, $number, $date )
	{
		...
		switch ( strtolower( $datepart ) ) 
		{
			...
			default:
				return PEAR::raiseError( "Unsupported operation." );
		}
	}
	...
	function ActualActualyearFraction( $startdate, $enddate, $refPeriodStart = null, $refPeriodEnd = null )
	{
		...
		if ( $startdate > $enddate ) 
			return PEAR::raiseError( "Invalid dates." );

		if ( !( $refPeriodStart != $t ) && ( $refPeriodEnd != $t ) && ( $refPeriodEnd > $refPeriodStart ) && ( $refPeriodEnd > $startdate ) ) 
			return PEAR::raiseError( "Invalid reference period." );
				
		$months = intval( 0.5 + 12 * $this->DATEDIFF( 'day', $refPeriodStart, $refPeriodEnd) / 365 );
		$period = $months / 12.0;
		
		if ( $months == 0 ) 
			return PEAR::raiseError( "Number of months does not divide 12 exactly." );
		...
		if ( $enddate <= $refPeriodEnd ) 
		{
			...
			if ( $startdate >= $refPeriodStart ) 
			{
				...
				return $period * $this->ActualActualdayCount( $startdate, $enddate ) / $this->ActualActualdayCount( $refPeriodStart, $refPeriodEnd );
			} 
			else 
			{
				...
				if ( $enddate > $refPeriodStart )
					return $this->ActualActualyearFraction( $startdate, $refPeriodStart, $previousRef, $refPeriodStart ) + $this->ActualActualyearFraction( $refPeriodStart, $enddate, $refPeriodStart, $refPeriodEnd );
				else
		...
		else 
		{
			...
			$sum += $this->ActualActualyearFraction( $newRefStart, $d2, $newRefStart, $newRefEnd );
			return $sum;
		}
	}
	 ...
	 */
	function VDB( $cost, $salvage, $life, $start_period, $end_period, $factor = 2, $no_switch = false )
	{
	...
	function PV( $rate, $nper, $pmt, $fv = 0.0, $type = 0 )
	{
		...
		else 
		{
			...
			$pv = ( $pmt * ( 1 + $rate * $type ) * ( 1 - $r ) / $rate - $fv ) / $r;
		}
	...
	function FV( $rate, $nper, $pmt, $pv = 0.0, $type = 0 )
	{
		...
		else 
		{
			...
			$fv = ( $pmt * ( 1 + $rate * $type ) * ( 1 - $r ) / $rate - $pv * $r );
		}
	...
	function FVSCHEDULE( $principal, $schedule )
	{
		...
		foreach ( $schedule as $rate )
			$principal = $this->FV( $rate, 1, 0, -$principal );
	...
	function PMT( $rate, $nper, $pv, $fv = 0.0, $type = 0 )
	{
		...
		else 
		{
			...
			$pmt = ( $fv + $pv * $r ) / ( ( 1 + $rate * $type ) * ( 1 - $r ) / $rate );
		}
	...
	function IPMT( $rate, $per, $nper, $pv, $fv = 0.0, $type = 0 )
	{
		...
		else 
		{
			...
			$ipmt = ( $fv + $pv * ( $r - $r2 ) ) / ( ( 1 + $rate * $type ) * ( 1 - $r ) / $rate );
		}
	...
	function PPMT( $rate, $per, $nper, $pv, $fv = 0.0, $type = 0 )
	{
		...
		else 
		{
			...
			$ppmt = ( $fv + $pv * $r2 ) / ( ( 1 + $rate * $type ) * ( 1 - $r ) / $rate );
		}
		
		return ( is_finite( $ppmt )? $ppmt: null );
	}
	 ...
	 */
	function DISC( $settlement, $maturity, $pr, $redemption, $basis = 0 )
	{
	 ...
	 */
	function RECEIVED( $settlement, $maturity, $investment, $discount, $basis = 0 )
	{
	 ...
	 */
	function INTRATE( $settlement, $maturity, $investment, $redemption, $basis = 0 )
	{
	 ...
	 *  4 European 30/360
	 */
	function YIELD( $settlement, $maturity, $rate, $pr, $redemption, $frequency, $basis = 0 )
	{
		if ( $rate < 0 ) 
			return null;
	 ...
	 */
	function PRICEDISC( $settlement, $maturity, $discount, $redemption, $basis = 0 )
	{
		if ( ( $basis < 0 ) || ( $basis > 4 ) ) 
			return null;
	 ...
	 */
	function YIELDDISC( $settlement, $maturity, $pr, $redemption, $basis = 0 )
	{
		if ( ( $basis < 0 ) || ( $basis > 4 ) ) 
			return null;
	 ...
	 */
	function COUPNUM( $settlement, $maturity, $frequency, $basis = 0 )
	{
		if ( ( $basis < 0 ) || ( $basis > 4 ) ) 
			return null;
	 ...
	 * the settlement date.
	 */
	function COUPDAYBS( $settlement, $maturity, $frequency, $basis = 0 )
	{
		if ( ( $basis < 0 ) || ( $basis > 4 ) ) 
			return null;
