 ...
 *
 * $decode = new MailMimeDecode( $input, "\r\n" );
 ...
 * @todo Implement decoding of bodies (Just need the _decodeBodies method writing)
 * @package peer_mail
 */

class MailMimeDecode extends PEAR
{
    /**
     ...
     * @access public
     */
    function MailMimeDecode( $input, $crlf = "\r\n" )
    {
		$this->_crlf = $crlf;
    ...
    function decode( $params = null )
    {
        ...
        if ( !isset( $this ) && isset( $params['input'] ) )
		{
            if ( isset( $params['crlf'] ) )
                $obj = new MailMimeDecode( $params['input'], $params['crlf'] );
            else
                $obj = new MailMimeDecode( $params['input'] );
		...
		else
		{
            ...
            if ( $structure === false || PEAR::isError( $structure ) )
            ...
            switch ( strtolower( $content_type['value'] ) )
			{
                ...
                case 'multipart/mixed':
                    ...
                    for ( $i = 0; $i < count( $parts ); $i++ )
					{
                        ...
                        if ( $part === false || PEAR::isError( $part ) )
                            return PEAR::raiseError( $this->_error );
                ...
                case 'message/rfc822':
                    $obj = new MailMimeDecode( $body, $this->_crlf );
