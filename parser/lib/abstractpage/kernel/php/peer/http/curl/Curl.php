...
class Curl extends PEAR
{
	 ...
	 *
	 * @return object Curl $obj   A new Curl object
	 */
	function Curl( $url = "" )
	{
		if ( $url )
	...
	function execute()
	{
		...
		if ( !$ret )
			return PEAR::raiseError( curl_error( $ch ), curl_errno( $ch ) );
		
		return( $ret );
	}

	/**
	 * Closes the curl transfer and finishes the object
	 */
	function close()
	{
		if ( $this->_ch )
