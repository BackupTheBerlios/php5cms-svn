...
using( 'image.text.ImageTextLine' );
	
	
define( "IMAGETEXTREGEX_HTMLCOLOR", "/^.*([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$/", true );
define( "IMAGETEXTALIGN_LEFT",      "left",   true );
define( "IMAGETEXTALIGN_RIGHT",     "right",  true );
define( "IMAGETEXTALIGN_CENTER",    "center", true );


/**
 * A generic class for text operations on images.
 *
 * Requires GD lib and Freetype.
 *
 * @package image_text
 */

class ImageText extends PEAR
{
	/**
	 ...
	 */
	 var $options = array(
		...
		'color'             => array( 'r' => 255, 'g' => 255, 'b' => 255 ), // OPTIONAL: Colorcode @see ImageText::colorize()
		'bgcolor'           => false,           // OPTIONAL: Colorcode for background color of the box @see ImageText::colorize() (needed for addBorder)
		'minFontSize'       => 1,               // OPTIONAL: Measurizing, min fontSize @see ImageText::measurize()
		'maxFontSize'       => 100,             // OPTIONAL: Measurizing, max fontSize @see ImageText::measurize()
		'maxLines'          => 100,             // OPTIONAL: Measurizing, max number of lines
		'align'             => IMAGETEXTALIGN_LEFT, // OPTIONAL: Standard alignement (has to be of ALIGNMENT constants)
		...
		'shadowColor'       => false,           // OPTIONAL: Color for a shadow @see ImageText::colorize()
	 ...
	 */
	function ImageText( $text, $options )
	{
		if ( is_array( $text ) ) 
		{
			foreach ( $text as $key => $item ) 
			{
				if ( is_object( $item ) && is_a( $item, "ImageTextLine" ) )
					$this->lines[] = $item;
				else
					$this->lines[] = new ImageTextLine( $item, $this->options );
			}
		} 
		else 
		{
			$this->lines[] = new ImageTextLine( $text, $this->options );
		}
	 ...
	 *
	 * @return ImageTextLine The current line as refernce
	 */
	 ...
	 *
	 * @return ImageTextLine The next line as refernce
	 */
	function &nextLine()
	{
	 ...
	 *
	 * @return ImageTextLine The first line as refernce
	 */
	function &firstLine()
	{
		$this->_curent = 0;
	 ...
	 *
	 * @access public
	 * @return array string Array of ImageTextLine objects
	 */
	 ...
	 * neccessary, because lines may be of different height. Has to be changed when possible.
	 *
	 * @access private
	 ...
	 * @return array The translated color code
	 */
	function _translateColor( $color )
	{
		...
		if ( !is_array( $color ) && preg_match( IMAGETEXTREGEX_HTMLCOLOR, $color, $matches ) ) 
		{
			$color = array(
				'r' => hexdec( $matches[1] ),
				'g' => hexdec( $matches[2] ),
	...
	function &_testMeasure( &$lines, &$tokens, $font, $fontsize ) 
	{
		if ( !$lines[0] ) 
		{
			$lines[0] =& new ImageTextLine( array_shift( $tokens ), $this->options );
		...
		{
			$lines[count( $lines )] =& new ImageTextLine( $line->popToken(), $this->options );
