...
class Graph extends PEAR
{
	...
	function Graph()
	{
		$this->no_data = "No data available.";
	}
	...
	{
		// chart area is entire canvas
		$this->chart_area = array( 0, 0, $this->image_width, $this->image_height );

		// subtract 2 percent at the right side (for better looks)
		...
		// space vor descriptions, so move the whole bunch up
	...
	function SetPointSize( $p_point_size )
	{
		...
		if ( $this->point_shape == "diamond" || $this->point_shape == "triangle" )
		{
		...
		foreach ( $this->data_values as $row )
		{
			...
			foreach ( $row as $v )
			{
				...
				{	
					...
					if ( $colbarcount >= count( $this->bar_border_color ) )
						$colbarcount = 0;
