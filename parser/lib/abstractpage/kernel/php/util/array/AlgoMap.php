 ...
 * these arrays.
 *
 * See examples below to an overview of AlgoMap::map() usage.
 *
 * *IMPORTANT*
 *
 ...
 *
 * $result = AlgoMap::map('myfunc', $a_pam);
 *
 ...
 *
 * Example 2 : Using more than one array with AlgoMap::map()
 * 
 * <?php
 * function myfunc($a, $b)
 * {
 ...
 *
 * $result = AlgoMap::map('myfunc', $a_nbr, $b_nbr);
 *
 ...
 * ?>
 * 
 * Example 4 : Defining the function code inside the AlgoMap::map() call
 * 
 * <?php
 * $a_t = array("a","b","c","d");
 * $b_t = array("z","b","e","d");
 ...
 * $foo = new Foo();
 * $array = array('a','b','c');
 * $result = AlgoMap::map($foo, 'myCallback', $array);
 *
 * // $result = array('-a-', '-b-', '-c-');
 * ?>
 *  
...
class AlgoMap extends PEAR
{
	...
	function map()
	{
    	...
    	while ( count( $argv[0] ) > 0 ) 
		{
        	...
        	if ( isset( $obj ) ) 
			{
            	$result = array_merge( $result, call_user_func_array( array( $obj, $func ), $fargs ) );
        	} 
			else 
			{
            	// we merge the result of the function
            	// with allready found results
            	$result = array_merge( $result, call_user_func_array( $func, $fargs ) );
        	}
    	}
