...
XMLRPC.prototype = new Base();
XMLRPC.prototype.constructor = XMLRPC;
XMLRPC.superclass = Base.prototype;

/**
 * @access public
 * @static
 */
XMLRPC.routeServer = "http://www.docuverse.de/rpc/server.php";
 ...
 * @static
 */
XMLRPC.queue = new Array();

/**
 * @access public
 * @static
 */
XMLRPC.timers = new Array();

/**
 * @access public
 * @static
 */
XMLRPC.timeout = 30000;

/**
 * @access public
 * @static
 */
XMLRPC.ontimeout = null;
 ...
 * @static
 */	
XMLRPC.getService = function( serviceName )
{
 ...
 * @static
 */
XMLRPC.purge = function( modConst, receive )
{
	if ( this.stack[modConst.URL].length )
	{
		...
		{
			if ( modConst.onerror )
				modConst.onerror( this.lastError );
 ...
 * @static
 */
XMLRPC.revert = function( modConst )
{
...
XMLRPC.call = function()
{
	...
	{
		if ( service && service.onerror )
			service.onerror( this.lastError );
 ...
 * @access public
 */
XMLRPC.typematch = function( vDunno, nCase )
{
	var nMask;
 ...
 * @static
 */
XMLRPC.getNode = function( data, tree )
{
	var nc = 0; // nodeCount
...
XMLRPC.toObject = function( data )
{
	...
	switch ( data.tagName )
	{
		...
		// not sure if this is part of the xmlrpc specification
		case "time":
 ...
 * @static
 */
XMLRPC.decodeBase64 = function( sEncoded )
{
 ...
 * @static
 */
XMLRPC.getObject = function( type, message )
{
	if ( type == "HTTP" )
	{
		if ( Browser.apopt )
 ...
 * @static
 */
XMLRPC.validateMethodName = function( name )
{
	/*
	Do Checking:
 ...
 * @static
 */
XMLRPC.handleError = function( e )
{
	this.stop = true;
	this.lastError = e;
	
	if ( !this.onerror || !this.onerror( e ) )
		return Base.raiseError( e );
};
 ...
 * @static
 */
XMLRPC.send = function( serverAddress, functionName, args, receive, multicall, autoroute )
{
	var id, http;
	...
	try
	{
		...
		http.setRequestHeader( "User-Agent", "Abstractpage" );
 ...
 * @static
 */
XMLRPC.processResult = function( http )
{
	if ( http.status == 200 )
	{
